...
class FilePermissions {
    ...
    private function __construct() {
        ...
        if ( CoreOptions::read_option( 'theme_enforce_wpconfig_permissions' ) ) {
            ...
            if ( $permissions && 0440 !== $permissions ) {
    ...
    private function read_wp_config_permissions(): int {
        FileUtils::wp_filesystem();
        if ( FileUtils::is_readable( ABSPATH . 'wp-config.php' ) ) {
            return (int) ( fileperms( ABSPATH . 'wp-config.php' ) & 0777 );
        }
        return 0;
    }
    ...
    private function change_wp_config_permissions(): bool {
        ...
        $success = chmod( ABSPATH . 'wp-config.php', 0440 );
        if ( $success ) {
    ...
    public function ajax_get_file_status(): void {
        ...
        $wpconfig_secure    = ( 0440 === $wpconfig_perms || 0400 === $wpconfig_perms );
        ...
        wp_send_json_success( array(
            ...
            'wpconfig_perms'    => $wpconfig_perms ? decoct( $wpconfig_perms ) : null,
    ...
    public function ajax_protect_uploads_dir(): void {
        ...
        $results[] = $this->get_uploads_nginx_snippet( $upload_dir['baseurl'] );
...
WEBCONFIG;
    ...
    private function get_uploads_nginx_snippet( string $uploads_url ): string {
        $uploads_path_rel = parse_url( $uploads_url, PHP_URL_PATH );
        return sprintf(
