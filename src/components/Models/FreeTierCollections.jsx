...
export default function FreeTierCollections( { form, setField, postTypes } ) {
	const { __ } = wp.i18n || {};
	...
	return (
		<Stack spacing={ 3 } maxWidth={ 600 } sx={ { p: 4, pt: 6 } }>
			...
			<Tooltip
				title={ ! hasValidLicense ? __( 'Upgrade to Pro to enable sorting', 'rest-api-firewall' ) : '' }
				followCursor
			>
				<Stack spacing={ 3 }>
					<Stack
					...
					>
						<FormControl sx={ { flex: 1 } } disabled={ ! hasValidLicense }>
							<FormControlLabel
								control={
									<Switch
										checked={ !! form.rest_collections_sortable_enabled }
										onChange={ setField }
										size="small"
						...
						<Box sx={ { flex: 1 } }>
							<MultipleSelect
								...
								label={ __( 'Choose Post Types', 'rest-api-firewall' ) }
								value={ form.rest_collections_sortable_post_types ?? [] }
								helperText={ __( 'Enable sorting on the selected post types.', 'rest-api-firewall' ) }
							...
							<MultipleSelect
								...
								label={ __( 'Choose Taxonomies', 'rest-api-firewall' ) }
								value={ form.rest_collections_sortable_taxonomies ?? [] }
								helperText={ __( 'Enable term ordering via REST for the selected taxonomies.', 'rest-api-firewall' ) }
								disabled={ ! isProActive }
					...
					<Stack spacing={ 3 }>
						<FormControl disabled={ ! isProActive }>
							<FormControlLabel
								control={
									<Switch
										checked={ !! form.rest_collections_sortable_rest_enforce }
										onChange={ setField }
										size="small"
							...
							/>
							<FormHelperText>
								{ __( 'Applies in requests with no orderby parameter.', 'rest-api-firewall' ) }
							</FormHelperText>
						</FormControl>
						...
						<FormControl disabled={ ! isProActive }>
							<FormControlLabel
								control={
									<Switch
										checked={ !! form.rest_collections_sortable_wp_query_enforce }
										onChange={ setField }
										size="small"
							...
							/>
							<FormHelperText>
								{ __( 'Applies in queries with no orderby argument.', 'rest-api-firewall' ) }
							</FormHelperText>
						</FormControl>
					</Stack>
				</Stack>
