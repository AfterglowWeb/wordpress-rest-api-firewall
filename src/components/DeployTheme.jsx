...
export default function DeployTheme( { setStatus, status } ) {
	...
	const handleDeploy = useCallback( async () => {
		...
		try {
			...
			const hasError = result.steps?.some(
				( s ) => s.status === STEP_STATUS.error
			);
			if ( hasError ) {
		...
		<Stack maxWidth="sm" my={ 3 }>
			<Box>
				...
				>
					...
					{ status && ! status?.active
						...
						: __( 'Theme Status', 'rest-api-firewall' ) }
				</Typography>

				{ status && ! status?.active && (
					<Alert severity="info" sx={ { my: 3 } }>
						{ __(
							'REST API Firewall Theme is bundled with the plugin.',
							'rest-api-firewall'
						) }
						<br />
						{ __(
							'You need to deploy and activate it to access theme options.',
							'rest-api-firewall'
						) }
					</Alert>
				) }
							...
							<Stack spacing={ 2 }>
								...
								{ status.deployed &&
									...
									status.bundled_version !==
										...
										>
											{ __(
												'Update available: Deployed version',
												'rest-api-firewall'
											) }{ ' ' }
											({ status.deployed_version }) → (
											{ status.bundled_version })
									...
									! status.current_theme.is_ours && (
										<Box>
											...
											<Typography variant="body2">
												{ status.current_theme.name } (v
												{ status.current_theme.version }
												)
											</Typography>
										</Box>
									) }
			...
			<Collapse in={ expanded || ! status?.deployed }>
				<Box sx={ { py: 3, pb: 2 } }>
					{ status && ! status.deployed && (
						...
						>
							{ deploying
								? __( 'Deploying…', 'rest-api-firewall' )
								: __( 'Deploy Theme', 'rest-api-firewall' ) }
						</Button>
					) }
					...
					{ status && status.deployed && ! status.active && (
						<Stack direction="row" spacing={ 1 }>
							...
							>
								{ activating
									? __( 'Activating…', 'rest-api-firewall' )
									: __(
											'Activate Theme',
											'rest-api-firewall'
									  ) }
			...
			>
				...
				<DialogContent>
					<Typography>
						{ __(
							'The theme has been deployed. Would you like to activate it now?',
							'rest-api-firewall'
						) }
					</Typography>
				</DialogContent>
			...
			>
				...
				<DialogContent>
					<Typography>
						{ __(
							'The theme has been activated successfully. You can now configure theme options below.',
							'rest-api-firewall'
						) }
					</Typography>
				</DialogContent>
			...
			>
				...
				<DialogContent>
					<Typography>
						{ __(
							'This will overwrite the current theme files with the bundled version. Are you sure you want to continue?',
							'rest-api-firewall'
						) }
					</Typography>
				</DialogContent>
			...
			>
				...
				<DialogContent>
					<Typography>
						{ redeploySuccess
							? __(
									'The theme has been redeployed successfully.',
									'rest-api-firewall'
							  )
							: __(
									'There was an error redeploying the theme. Please check the error message and try again.',
									'rest-api-firewall'
							  ) }
					</Typography>
				</DialogContent>
